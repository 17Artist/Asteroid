 ...
 */
package priv.seventeen.artist.asteroid.nms.v1_20_4;
...
public class ItemTagBridgeImpl implements ItemTag.Bridge {
    ...
    private ItemTagData nbtToItemTagData(Tag nbt) {
        return switch (nbt.getId()) {
            case Tag.TAG_BYTE -> ItemTagData.of(((ByteTag) nbt).getAsByte());
            case Tag.TAG_SHORT -> ItemTagData.of(((ShortTag) nbt).getAsShort());
            case Tag.TAG_INT -> ItemTagData.of(((IntTag) nbt).getAsInt());
            case Tag.TAG_LONG -> ItemTagData.of(((LongTag) nbt).getAsLong());
            case Tag.TAG_FLOAT -> ItemTagData.of(((FloatTag) nbt).getAsFloat());
            case Tag.TAG_DOUBLE -> ItemTagData.of(((DoubleTag) nbt).getAsDouble());
            case Tag.TAG_STRING -> ItemTagData.of(nbt.getAsString());
            ...
            default -> ItemTagData.of(nbt.getAsString());
        };
    }
